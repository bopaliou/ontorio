...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (in_array(self::COL_ENTITE_TYPE, $columns) && in_array('entite_type', $columns)) {
            DB::statement('UPDATE documents SET entite_type = `entité_type` WHERE entite_type IS NULL OR entite_type = ""');
        }
        if (in_array(self::COL_ENTITE_ID, $columns) && in_array('entite_id', $columns)) {
        ...
        Schema::table('documents', function (Blueprint $table) use ($columns) {
            if (in_array(self::COL_ENTITE_TYPE, $columns)) {
                $table->dropColumn(self::COL_ENTITE_TYPE);
            }
            if (in_array(self::COL_ENTITE_ID, $columns)) {
                $table->dropColumn(self::COL_ENTITE_ID);
            }
        });
    }
