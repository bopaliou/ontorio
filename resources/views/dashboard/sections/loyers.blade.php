<div class="flex flex-col gap-6" id="loyers-section-container">
    ...
    <div id="loy-view-list" class="loy-sub-view space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-[#274256]">Suivi des Loyers</h2>
                <p class="text-sm text-gray-500 mt-1">Pointage des quittances et gestion des impayés.</p>
            </div>
            @if(App\Helpers\PermissionHelper::can('loyers.generate'))
            <div class="flex gap-3">
                <form id="loy-gen-form" method="POST" action="{{ route('loyers.genererMois') }}" target="loy_post_target">
                    @csrf
        ...
        <div id="loy-kpi-container" class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="bg-white p-5 rounded-3xl shadow-sm border border-gray-100">
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Facturé (Mois)</p>
                @php
                ...
                @endphp
                <p class="text-2xl font-black text-[#274256] mt-1">{{ number_format($factureMois, 0, ',', ' ') }} F</p>
            </div>
            <div class="bg-white p-5 rounded-3xl shadow-sm border border-gray-100">
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Encaissé (Mois)</p>
                <p class="text-2xl font-black text-green-600 mt-1">{{ number_format($encaisseMois, 0, ',', ' ') }} F</p>
            </div>
            <div class="bg-white p-5 rounded-3xl shadow-sm border border-red-50 border-l-4 border-l-red-500">
                <p class="text-[10px] font-black text-red-400 uppercase tracking-widest">Impayés Global</p>
                <p class="text-2xl font-black text-red-600 mt-1">{{ number_format($retardTotal, 0, ',', ' ') }} F</p>
            </div>
            <div class="bg-white p-5 rounded-3xl shadow-sm border border-gray-100">
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Taux Collecte</p>
                <p class="text-2xl font-black text-blue-600 mt-1">{{ $factureMois > 0 ? round(($encaisseMois / $factureMois) * 100) : 0 }}%</p>
            </div>
        </div>

        <!-- Table -->
        <div id="loy-table-container" class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase">Période</th>
                            <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase">Locataire & Bien</th>
                            <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase text-right">Montant</th>
                    ...
                    <tbody class="divide-y divide-gray-50 text-sm font-medium">
                        ...
                        <tr class="hover:bg-gray-50/80 transition group">
                            <td class="px-6 py-4">
                                ...
                                <div class="text-[9px] text-gray-400 font-bold uppercase mt-0.5">Échéance: {{ \Carbon\Carbon::parse($loy->mois)->endOfMonth()->format('d/m/Y') }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-gray-700 font-bold capitalize">{{ $loy->contrat->locataire->nom ?? 'Inconnu' }}</div>
                                <div class="text-[10px] text-gray-400 italic">Bien: {{ $loy->contrat->bien->nom ?? 'Unité' }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                @endphp
                                <span class="px-2.5 py-1 {{ $cls }} text-[9px] uppercase font-black rounded-full">
                                    {{ $st }}
                                </span>
                                @if($loy->statut === 'annulé' && $loy->note_annulation)
                                    <div class="text-[9px] text-gray-400 mt-1 italic max-w-[120px] truncate mx-auto" title="{{ $loy->note_annulation }}">
                                        "{{ $loy->note_annulation }}"
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <button onclick="loySection.openPaymentModal({{ json_encode($loy) }})" class="bg-[#274256] text-white px-3 py-1.5 rounded-lg text-[10px] font-black uppercase shadow-sm hover:scale-105 transition">
                                        Régler
                                    </button>
                                    @endif
                                    ...
                                    @if($loy->statut === 'payé')
                                    <button onclick="window.previewDoc({url: '{{ route('loyers.quittance', $loy->id) }}', nom_original: 'Quittance_{{ str_replace([' ', "'"], ['_', '_'], $loy->contrat->locataire->nom) }}.pdf', type_label: 'Quittance de Loyer'})" class="p-2 text-gray-400 hover:text-red-500 transition" title="Visualiser la Quittance (PDF)">
                        ...
                        @empty
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center text-gray-400 italic">Aucune quittance émise.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div id="loy-payment-modal" onclick="if(event.target === this) loySection.closePaymentModal()" class="fixed inset-0 z-[100] hidden bg-slate-900/40 backdrop-blur-md transition-all duration-300 flex items-center justify-center p-4">
        <div id="loy-payment-container" class="bg-white w-full sm:max-w-xl rounded-2xl shadow-2xl transform scale-95 opacity-0 transition-all duration-300 overflow-hidden border border-gray-100">
            ...
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white">
                <div>
                    <h3 class="text-base font-bold text-[#274256]">Enregistrer un Paiement</h3>
                    <p class="text-[10px] text-gray-500 mt-0.5 font-medium">Solder la quittance.</p>
                </div>
                <button onclick="loySection.closePaymentModal()" class="text-gray-400 hover:text-gray-600 hover:bg-gray-100 p-1.5 rounded-full transition">
            ...
            <form id="loy-payment-form" action="{{ route('paiements.store') }}" method="POST" target="loy_post_target" class="p-6 space-y-4" enctype="multipart/form-data">
                ...
                <div class="bg-blue-50/50 border border-blue-100 rounded-xl p-3 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-[9px] font-black text-blue-400 uppercase tracking-widest">Locataire</p>
                            <p class="text-xs font-bold text-gray-900 capitalize truncate" id="loy-payment-locataire">...</p>
                            <p class="text-[10px] text-gray-500 font-medium" id="loy-payment-periode">...</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-[9px] font-black text-blue-400 uppercase tracking-widest">À Payer</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="relative bg-gray-50 rounded-xl border border-gray-200 px-3 py-2 focus-within:ring-2 focus-within:ring-green-500/10 focus-within:border-green-600 transition-all">
                        <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Montant Encaissé</label>
                ...
                <div class="relative bg-gray-50 rounded-xl border border-gray-200 px-3 py-2 focus-within:ring-2 focus-within:ring-blue-500/10 focus-within:border-blue-600 transition-all">
                     <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Justificatif (PDF/Image)</label>
                     <input type="file" name="preuve" class="block w-full text-[10px] text-gray-500 file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-[10px] file:font-black file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer">
                </div>
    ...
    <div id="loy-edit-modal" class="fixed inset-0 z-[105] hidden bg-slate-900/40 backdrop-blur-md transition-all duration-300 flex items-center justify-center p-4">
        <div id="loy-edit-container" class="bg-white w-full max-w-sm rounded-2xl shadow-2xl transform scale-95 opacity-0 transition-all duration-300 border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white">
                <div>
                    <h3 class="text-sm font-bold text-[#274256]">Ajustement Loyer</h3>
                    <p class="text-[9px] text-gray-500 font-medium">Modification manuelle.</p>
                </div>
            ...
            <form id="loy-edit-form" class="p-6 space-y-4">
                ...
                <div class="relative bg-gray-50 rounded-xl border border-gray-200 px-3 py-2 focus-within:ring-2 focus-within:ring-blue-500/10 focus-within:border-blue-600 transition-all">
                    <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Montant Loyer</label>
                ...
                <div class="relative bg-gray-50 rounded-xl border border-gray-200 px-3 py-2 focus-within:ring-2 focus-within:ring-blue-500/10 focus-within:border-blue-600 transition-all">
                    <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Statut Actuel</label>
                ...
                <div id="loy-edit-note-group" class="hidden relative bg-red-50/50 rounded-xl border border-red-100 px-3 py-2 focus-within:ring-2 focus-within:ring-red-500/10 focus-within:border-red-600 transition-all">
                    <label class="block text-[9px] font-black text-red-400 uppercase tracking-widest mb-0.5">Motif d'annulation</label>
                ...
                <div id="loy-edit-partial-group" class="hidden relative bg-orange-50/50 rounded-xl border border-orange-100 px-3 py-2 focus-within:ring-2 focus-within:ring-orange-500/10 focus-within:border-orange-600 transition-all">
                    <label class="block text-[9px] font-black text-orange-400 uppercase tracking-widest mb-0.5">Montant déjà encaissé</label>
...
<script>
    window.loySection = {
        ...
        onStoreSuccess: function(msg) {
            ...
            refreshIframe.src = '{{ route('dashboard') }}';
            refreshIframe.onload = () => {
                ...
                setTimeout(() => {
                    ...
                    if(btn) {
                        ...
                        btn.disabled = false;
                        document.getElementById('loy-payment-form').reset();
                    }
                    if(genBtn && genText) {
                        genText.innerHTML = 'Générer Mois Actuel';
                        genBtn.classList.replace('border-green-500', 'border-gray-100');
